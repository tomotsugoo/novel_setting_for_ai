...
現在 **計11ツール**（ヘルプ1 + 読み取り6 + 統合4）。先頭に `help` を置き、自己文書化している。

### ヘルプ・読み取り・検証系（先頭7つ・常に可視に保つ）

| ツール | 引数 | 説明 |
|---|---|---|
| `help` | なし | このMCPの使い方を返す（全ツール・各統合ツールのaction一覧・引数・執筆のコツ）。`getHelp()` が `TOOLS` から動的生成するので一覧はドリフトしない |
| `get_character` | `id`, `scene_time?` | キャラ情報＋指定時刻での意識状態 |
| `list_overview` | なし | 全シーン一覧＋全キャラ一覧をまとめて返す（`list_scenes`＋`list_characters` を統合） |
